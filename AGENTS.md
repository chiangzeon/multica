...
> **Single source of truth:** `CLAUDE.md` at the project root is authoritative.
> Read it first. This file mirrors its current rules for Codex and other agents
> that discover `AGENTS.md`. If the two files differ, follow `CLAUDE.md`.
> Use `Makefile`, `package.json`, and `pnpm-workspace.yaml` as the source of
> truth for available commands.
## Conventions
